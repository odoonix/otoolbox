...
def init():
    ...
    for agent_file in AGENT_FILES:
        env.add_resource(
            ...
            init=[utils.constructor_copy_resource(f"addons/copilot/{agent_file}")],
            update=[utils.touch_file, utils.constructor_copy_resource(f"addons/copilot/{agent_file}")],
    ...
    env.add_resource(
        ...
        init=[utils.constructor_copy_resource(f"addons/copilot/{COPILOT_INSTRAUCTIONS_WORKSPACE}")],
        update=[utils.touch_file, utils.constructor_copy_resource(f"addons/copilot/{COPILOT_INSTRAUCTIONS_WORKSPACE}")],
        destroy=[utils.delete_file],
        verify=[utils.is_file, utils.is_readable],
    ...
    env.add_resource(
        ...
        init=[utils.constructor_copy_resource(f"addons/copilot/{COPILOT_INSTRAUCTIONS_ODOONIX}")],
        update=[utils.touch_file, utils.constructor_copy_resource(f"addons/copilot/{COPILOT_INSTRAUCTIONS_ODOONIX}")],
        destroy=[utils.delete_file],
        verify=[utils.is_file, utils.is_readable],
